...
function origen(req, res) {
    ...
    if(!from) {
        return res.status(400).json({error: 'se necesita introducir "from"'})
    }
...
function destino(req, res) {
    ...
    if(!to) {
        return res.status(400).json({error: 'se requiere introducir "to"'})
    }
...
function mensaje(req, res) {
    ...
    if(!from|!to|!subject || !message) {
        return res.status(400).json({error: "falta introducir 'from', 'to', 'subject' o 'message'"})
    }
